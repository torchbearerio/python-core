...
class Hit(Base):
    ...
    def set_start_time_for_task(self, task, t=None):
        if not t:
            t = int(round(time.time() * 1000))
        key = "{task}_start".format(task=task)
        self.timeline = dict((self.timeline if self.timeline else {}).items() + {key: t}.items())

    def set_end_time_for_task(self, task, t=None):
        if not t:
            t = int(round(time.time() * 1000))
